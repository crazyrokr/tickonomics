...
class DeRoundingFilterSpec extends Specification {
    ...
    def "given tick at round mark with volume spike, when filter, then volume smoothed"() {
        given:
            def tick = new TickData(ROUND_MARK, "SPY", 450.50, 10000, new int[0])
            def recent = [
                new TickData(ROUND_MARK.minusSeconds(5), "SPY", 450.48, 100, new int[0]),
                new TickData(ROUND_MARK.minusSeconds(3), "SPY", 450.49, 120, new int[0]),
                new TickData(ROUND_MARK.minusSeconds(1), "SPY", 450.50, 90, new int[0])
            ]
    ...
    def "given tick not at round mark, when filter, then unchanged"() {
        given:
            def tick = new TickData(NON_ROUND, "SPY", 450.50, 10000, new int[0])
            def recent = [
                new TickData(NON_ROUND.minusSeconds(1), "SPY", 450.49, 100, new int[0])
            ]
    ...
    def "given tick at round mark without spike, when filter, then unchanged"() {
        given:
            def tick = new TickData(ROUND_MARK, "SPY", 450.50, 100, new int[0])
            def recent = [
                new TickData(ROUND_MARK.minusSeconds(2), "SPY", 450.49, 95, new int[0]),
                new TickData(ROUND_MARK.minusSeconds(1), "SPY", 450.50, 105, new int[0])
            ]
    ...
    def "given insufficient context ticks, when filter, then unchanged"() {
        given:
            def tick = new TickData(ROUND_MARK, "SPY", 450.50, 10000, new int[0])
            def recent = [
                new TickData(ROUND_MARK.minusSeconds(1), "SPY", 450.50, 100, new int[0])
            ]
