...
public class FredClient {
    ...
    public FredClient(RestClient.Builder restClientBuilder,
    ...
    public List<FredObservation> fetchSeries(String seriesId) {
        String url = baseUrl + "/series/observations?series_id={seriesId}&api_key={apiKey}&file_type=json&sort_order=desc&limit=10";
        ...
        return response.observations().stream()
                ...
                .map(obs -> new FredObservation(
                        LocalDate.parse(obs.date(), FRED_DATE_FMT).atStartOfDay().toInstant(java.time.ZoneOffset.UTC),
                        seriesId,
