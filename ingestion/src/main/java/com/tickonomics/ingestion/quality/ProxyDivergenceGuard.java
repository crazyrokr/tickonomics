...
public class ProxyDivergenceGuard {
  ...
  public DivergenceResult checkDivergence() {
    ...
    if (correlation < MIN_CORRELATION_THRESHOLD || divergenceScore > DIVERGENCE_THRESHOLD_STD) {
      double tbillLatest = tbillRates
          .getLast()
          .value();
      double sofrLatest = sofrRates
          .getLast()
          .value();
      ...
      log.warn(
          "Proxy divergence detected: correlation={}, score={}, tbill={}, sofr={}",
          correlation,
          divergenceScore,
          tbillLatest,
          sofrLatest);
