...
public class TalibAdapter {
    ...
    public BBandsResult computeBollingerBands(double[] data, int period, double devUp, double devDown) {
        requireNonEmpty(data);
        BandsResult result = (BandsResult) Bbands.execute(0, data.length - 1, data, period, devUp, devDown, MAType.TA_MAType_SMA.idx());
        return new BBandsResult(result.outRealUpperBand(), result.outRealMiddleBand(), result.outRealLowerBand(), result.outBegIdx(), result.outNBElement());
    }
