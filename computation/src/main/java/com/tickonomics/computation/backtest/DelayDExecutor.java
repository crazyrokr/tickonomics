...
public class DelayDExecutor {
    ...
    public List<BacktestResult> compareDelays(String strategyName,
        ...
        if (delay1.sharpeRatio() != 0 && Math.abs(delay0.sharpeRatio() / delay1.sharpeRatio()) > FRAGILITY_THRESHOLD) {
            delay1 = new BacktestResult(
                delay1.strategyName(), delay1.delay(), delay1.sharpeRatio(),
                delay1.totalReturn(), delay1.maxDrawdown(), delay1.winRate(),
                delay1.idealReturn(), delay1.adjustedReturn(), delay1.slippageCostBps(),
                true, delay1.tradeLog()
            );
        }
    ...
    private BacktestResult executeWithDelay(String strategyName, ExecutionDelay delay,
                                             List<Double> signalReturns,
                                             List<Double> executionPrices) {
        ...
        return new BacktestResult(
            strategyName, delay, sharpe, cumReturn, maxDD,
            n > 0 ? (double) wins / n : 0.0,
            cumReturn, cumReturn, 0.0, false, List.of()
        );
    }
}
