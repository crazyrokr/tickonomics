...
public class ArrowIpcTransport {
  ...
  public TimeSeriesBatch deserializeTimeSeries(byte[] arrowData) {
    try (ByteArrayInputStream bais = new ByteArrayInputStream(arrowData); ArrowStreamReader reader =
    ...
    public TimeSeriesBatch {
      ...
      if (values.length != timestamps.length) {
        throw new IllegalArgumentException(
            "Values length (" + values.length + ") must equal timestamps length (" + timestamps.length + ")");
      }
    }
  }
