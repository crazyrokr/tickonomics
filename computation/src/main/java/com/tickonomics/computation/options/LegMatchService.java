...
public class LegMatchService {
        ...
        for (var entry : byExpiry.entrySet()) {
            ...
            for (int i = 0; i < sorted.size() - 2; i++) {
                for (int j = i + 2; j < sorted.size(); j++) {
                    double k1 = sorted.get(i).strike().doubleValue();
                    double k3 = sorted.get(j).strike().doubleValue();
            ...
            for (int i = 0; i < sorted.size() - 3; i++) {
                for (int l = i + 3; l < sorted.size(); l++) {
                    for (int j = i + 1; j < l - 1; j++) {
                        for (int k = j + 1; k < l; k++) {
                            double k1 = sorted.get(i).strike().doubleValue();
                            double k2 = sorted.get(j).strike().doubleValue();
                            double k3 = sorted.get(k).strike().doubleValue();
                            double k4 = sorted.get(l).strike().doubleValue();
    ...
    public List<LegGroup> findVerticalSpreads(String underlying, List<CdmOptionSnapshot> chain) {
        ...
        for (var entry : byExpiry.entrySet()) {
            ...
            for (int i = 0; i < sorted.size() - 1; i++) {
                clusters.add(new LegGroup(
                    List.of(sorted.get(i), sorted.get(i + 1)),
    ...
    private int findStrikeInRange(List<CdmOptionSnapshot> sorted, int lo, int hi, double target) {
        for (int i = lo; i <= hi; i++) {
            if (Math.abs(sorted.get(i).strike().doubleValue() - target) < STRIKE_TOLERANCE) {
                return i;
            }
        }
        return -1;
