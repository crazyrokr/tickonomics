...
public class DiagonalSpreadStrategy extends BaseOptionStrategy {
    ...
    protected double calculateFormula(LegGroup legs) {
        double farMid = midPrice(legs.components().get(0));
        double nearMid = midPrice(legs.components().get(1));
        double farDelta = Math.abs(legs.components().get(0).delta());
        double nearDelta = Math.abs(legs.components().get(1).delta());
