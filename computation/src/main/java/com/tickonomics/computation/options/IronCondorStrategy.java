...
public class IronCondorStrategy extends BaseOptionStrategy {
  ...
  protected double calculateFormula(LegGroup legs) {
    double longPutMid = midPrice(legs
        .components()
        .get(0));
    double shortPutMid = midPrice(legs
        .components()
        .get(1));
    double shortCallMid = midPrice(legs
        .components()
        .get(2));
    double longCallMid = midPrice(legs
        .components()
        .get(3));
    double netCredit = (shortPutMid - longPutMid) + (shortCallMid - longCallMid);
    double wingWidth = Math.abs(legs
        .components()
        .get(1)
        .strike()
        .doubleValue() - legs
