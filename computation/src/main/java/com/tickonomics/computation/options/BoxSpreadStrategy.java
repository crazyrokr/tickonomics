...
public class BoxSpreadStrategy extends BaseOptionStrategy {
    ...
    protected double calculateFormula(LegGroup legs) {
        double bullCallDebit = midPrice(legs.components().get(0)) - midPrice(legs.components().get(1));
        double bearPutDebit = midPrice(legs.components().get(2)) - midPrice(legs.components().get(3));
        ...
        double ttm = legs.components().get(0).ttmYears();
