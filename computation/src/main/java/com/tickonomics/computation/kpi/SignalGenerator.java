...
public class SignalGenerator {
    ...
    public Optional<SignalResult> evaluate(String symbol, IliResult iliResult, double[] historicalIliValues) {
        if (historicalIliValues == null || historicalIliValues.length < 20) {
            return Optional.of(new SignalResult(symbol, SignalResult.DIR_BUY, SignalResult.STATUS_INSUFFICIENT,
                    Double.NaN, iliResult.iliValue(), 0, 0, 0));
        }
        ...
        return Optional.of(new SignalResult(symbol, direction, status, percentile,
                iliResult.iliValue(), expectedMove, estimatedCost, strength));
    }
    ...
    boolean isOnCooldown(String symbol) {
        ...
        return last != null && Instant.now().isBefore(last.plusMillis(cooldownMs));
    }
