...
public class FixedIncomePortfolioBuilder {
    ...
    public FixedIncomePortfolio buildBullet(double targetDuration, List<BondPosition> availableBonds) {
        ...
        List<BondPosition> positions = List.of(new BondPosition(
            closest.tenor(), closest.duration(), closest.yield(), 1.0
        ));
    ...
    public FixedIncomePortfolio buildBarbell(double shortDuration, double longDuration,
        ...
        List<BondPosition> positions = List.of(
            new BondPosition(shortBond.tenor(), shortBond.duration(), shortBond.yield(), weightShort),
        ...
        double convexity = weightShort * Math.pow(shortBond.duration(), 2)
            + weightLong * Math.pow(longBond.duration(), 2);
    ...
    public FixedIncomePortfolio buildDurationNeutral(double shortDuration, double midDuration,
                                                      double longDuration, List<BondPosition> availableBonds) {
        BondPosition shortBond = findClosestByDuration(availableBonds, shortDuration);
