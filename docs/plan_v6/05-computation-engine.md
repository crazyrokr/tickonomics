...
**Cross-validation:** Via TA-Lib (in-process), FinanceToolkit (analytics worker), and Yahoo Finance
...
- Monitors the NBBO midpoint at the time a Finnhub trade event (e.g., "Odd Lot") appears.
...
Estimates trader type dominance from Finnhub trade event codes and order flow patterns.
...
- AT activity estimated via message intensity from Finnhub (`at_activity_proxy`).
...
### 50. LeverageSignaler (v5 -- Proposal 08: Leverage Rotation)

**Location:** `computation/src/main/java/com/tickonomics/computation/leverage/LeverageSignaler.java`

Monitors configurable MA window (default 200-day) of S&P 500 for systematic rotation between leveraged equity positions
and risk-free assets.

- Outputs `LEVERAGE_ON` / `LEVERAGE_OFF` signal to `SignalGenerator`.
- Integrates with `VirtualPortfolio` (Track 10) for automated position scaling.
- Truncates tail risk by rotating into Treasury bills during risk-off regimes.
- Default: 200-day SMA of SPY close prices.

### 51. PairsTradingEngine (v5 -- Proposal 08: Pairs Trading Verification)

**Location:** `computation/src/main/java/com/tickonomics/computation/pairs/PairsTradingEngine.java`

Minimum-distance pairs trading as an orthogonal signal verification tool.

- Distance-based matching engine identifies pairs and tracks price divergence.
- When ILI signals (funding liquidity) and pairs signals (relative value) diverge, indicates high probability of
  liquidity-driven market dislocation.
- Monitors "close substitutes" (e.g., ETFs of same sector or credit quality).

### 52. SaliProcessor (v5 -- Proposal 09: Sentiment-Augmented ILI)

**Location:** `computation/src/main/java/com/tickonomics/computation/signal/SaliProcessor.java`

Combines `ili_value` with `sentiment_score` for sentiment-augmented signal confirmation.

- Configurable weighting (e.g., 70% ILI / 30% sentiment).
- AND-gate logic: both branches must agree for signal confirmation.
- SELL signals suppressed if sentiment is extremely BULLISH despite high ILI.
- Accepts scores from either FinBERT or lexicon SALI via common interface.

### 53. SentimentVolatilityGuard (v5 -- Proposal 09: Sentiment-Volatility Interaction)

**Location:** `computation/src/main/java/com/tickonomics/computation/signal/SentimentVolatilityGuard.java`

Uses BERT-derived certainty scores to adjust `RegimeDetector` thresholds.

- If news is highly contradictory (low BERT certainty), widens ILI signal thresholds.
- High certainty in a direction narrows thresholds, increasing signal sensitivity.
- Complements existing `GarchRegimeDetector` with qualitative sentiment overlay.

### 54. ComparativeExecutionAnalysis (v5 -- Proposal 08: Execution Comparison)

**Location:** `computation/src/main/java/com/tickonomics/computation/execution/ComparativeExecutionAnalysis.java`

Supports passive (limit-order) vs. aggressive (market-order) execution class comparison.

- Includes "Fair Price" estimate (midpoint) in signal metadata for limit-order placement.
- Configuration flag toggles between `PASSIVE` and `AGGRESSIVE` execution modes.
- Dual portfolio comparison: Market-Order vs. Smart Limit-Order execution side-by-side.
- `PassiveExecutionHandler`: places limit orders at ILI-implied fair price +/- small markup.
- `SniperExecutionHandler`: reacts instantly to ILI threshold crossings.

### 55. PortfolioManagementAlgebra (v5 -- Proposal 08: Portfolio Algebra)

**Location:** `computation/src/main/java/com/tickonomics/computation/portfolio/PortfolioManagementAlgebra.java`

Standardizes rebalancing trades, leverage management, and margin-call mitigation.

- Integrates margin call equation into `VirtualPortfolio` rebalancing engine.
- Standardized linear cost model: `Trading costs = (t0 + t1)P + sP`.
- Iterative solution for margin adjustment converges efficiently in simulations.

### 56. AlgorithmicBehaviorAlignment (v5 -- Proposal 08: Behavior Alignment, selective)

**Location:** `computation/src/main/java/com/tickonomics/computation/signal/AlgorithmicBehaviorAlignment.java`

Selective adoption: time-decay function for `IntradayProxyService` and liquidity-adjusted slippage.

- `IntradayProxyService` includes time-decay function: as time to official SOFR publication (8:00 AM ET) decreases,
  signal threshold requirement increases proportionally.
...
### 57. MarkovStopEngine Dynamic Exits (v5 -- Proposal 08: Optimal Stops)

**Location:** `computation/src/main/java/com/tickonomics/computation/signal/MarkovStopService.java`

State-dependent stop-loss/take-profit derived from Markov-modulated diffusion.

- `SignalGenerator` and `PaperTradingEngine` support state-dependent exits.
- Stop levels adjust in real-time as strategy moves through its signal lifecycle.
- Consumes calibrated thresholds from Python worker `/stops/calibrate` endpoint.
- Replaces fixed stops with dynamic, data-driven thresholds.

### 58. AgnosticAggregator (v5 -- Proposal 10: Online Learning)

**Location:** `computation/src/main/java/com/tickonomics/computation/weight/AgnosticAggregator.java`

Regret-minimization weight aggregation using exponential weights (SSRN-5242085).

- Aggregates multiple weight-sets ("Macro-heavy", "Flow-heavy") using regret-minimization algorithm.
- Replaces static ILI weights with dynamically-weighted combination.
- Weight adaptation is stable — constrained to prevent erratic shifts.
- Self-optimizing system that reduces reliance on manual quarterly recalibration.

### 59. WalkForwardValidator (v5 -- Proposal 10: Walk-Forward Validation)

**Location:** `computation/src/main/java/com/tickonomics/computation/backtest/WalkForwardValidator.java`

Anchored walk-forward analysis for out-of-sample performance validation.

- Ensures Bayesian weight optimization does not overfit to historical noise.
- Evaluates performance based on regret in addition to Sharpe ratio.
- Out-of-sample segments maintained across all strategy performance metrics.
- Integrated with `WeightOptimizer` evaluation pipeline.

### 60. CrossModelValidator (v5 -- Proposal 10: Multi-Model Benchmark)

**Location:** `computation/src/main/java/com/tickonomics/computation/backtest/CrossModelValidator.java`

Tournament framework comparing ILI rule engine against ML models.

- Replays historical data through ILI, XGBoost, and LSTM benchmark models.
- Regime-specific benchmarking: uptrend, sideways, downtrend.
- Side-by-side performance charts for all model outputs.
- Validates that ILI approach outperforms black-box ML in its target domain.

### 61. VotingClassifier (v5 -- Proposal 10: Multi-Model Confirmation)

**Location:** `computation/src/main/java/com/tickonomics/computation/signal/VotingClassifier.java`

Optional ML confirmation filter for signal generation.

- Requires ILI signal to be "voted on" by at least one other ML model.
- Example: "ILI Buy" + "XGBoost Bullish" = `ACTIONABLE`.
- Optional `ML_CONFIRMATION` filter in `SignalGenerator`.
- Disabled by default; enabled via configuration for A/B testing.

### 62. MarketEfficiencyMonitor (v5 -- Proposal 11: Efficiency Gap)

**Location:** `computation/src/main/java/com/tickonomics/computation/kpi/MarketEfficiencyMonitor.java`

Fundamental-algorithmic gap indicator bridging macro liquidity and high-frequency signals.

- Fundamental component: current ILI value (macro liquidity).
- Algorithmic component: high-frequency intraday volume imbalance.
- Gap highlights periods where price discovery is driven by AT noise vs. fundamental macro events.
- Leverages existing `AlgorithmicIntensityMetric` AT proxy data.

### 63. RiskPremiumResidualMonitor (v5 -- Proposal 12: Q-World Validation)

**Location:** `computation/src/main/java/com/tickonomics/computation/risk/RiskPremiumResidualMonitor.java`

Independent mathematical validation of `ProxyDivergenceGuard` DISLOCATED status.

- Compares observed market spread (P-world) against Q-world fair-value model output.
- Residual (Observed - Fair) exceeding defined threshold validates DISLOCATED status.
- Integrates with existing `ProxyDivergenceGuard` as a second confirmation layer.
- Consumes fair-value data from Python worker `/fixed-income/q-world-fair-value`.

### 64. MarketSensitivityLibrary (v5 -- Proposal 12: Greeks Nomenclature)

**Location:** `computation/src/main/java/com/tickonomics/computation/sensitivity/MarketSensitivityLibrary.java`

Standardized risk sensitivity framework with Greeks nomenclature.

- Maps existing KPIs to standardized nomenclature:

| Traditional Name       | Greeks Name  | Description                            |
|------------------------|--------------|----------------------------------------|
...
- `IliCalculator` and `SignalGenerator` reference metrics using standardized nomenclature.
- Signal cost model includes sensitivity-adjusted liquidity premium.
- High-duration bonds require larger risk premium for signal admissibility.
- Consumes analytical Greeks from Python worker `/fixed-income/tbill-greeks`.
...
- [ ] `LegMatchService` groups butterfly and condor legs from raw snapshots (v5.2).
- [ ] `UniverseAggregator` computes global mean returns for cross-sectional strategies (v5.2).
- [ ] `EvtRiskService` fits GPD parameters and computes tail-VaR for risk monitoring (v5.2).
- [ ] `QuantileBandService` produces dynamic, conditional liquidity bands from QR (v5.2).
- [ ] `SignalGenerator` enforces Descriptive vs. Inferential partitioning in outputs (v5.2).
...
| v5      | Added 8 new components from Proposals 08, 09: (1) LeverageSignaler — 200-day MA leverage rotation (Proposal 08). (2) PairsTradingEngine — minimum-distance pairs verification (Proposal 08). (3) SaliProcessor — sentiment-augmented ILI with AND-gate confirmation (Proposal 09). (4) SentimentVolatilityGuard — BERT certainty-based threshold adjustment (Proposal 09). (5) ComparativeExecutionAnalysis — passive vs aggressive dual execution (Proposal 08). (6) PortfolioManagementAlgebra — margin algebra and standardized cost model (Proposal 08). (7) AlgorithmicBehaviorAlignment — time-decay for IntradayProxyService (Proposal 08 selective). (8) MarkovStopService — state-dependent dynamic exits (Proposal 08). New modules: `leverage/`, `pairs/`, `execution/`, `portfolio/`. 8 new unit tests. 8 new validation criteria.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| v5.1    | Added 7 new components from Proposals 10, 11, 12: (1) AgnosticAggregator — regret-minimization weight aggregation (Proposal 10). (2) WalkForwardValidator — anchored walk-forward analysis for out-of-sample validation (Proposal 10). (3) CrossModelValidator — tournament framework ILI vs XGBoost vs LSTM (Proposal 10). (4) VotingClassifier — optional ML confirmation filter (Proposal 10). (5) MarketEfficiencyMonitor — fundamental-algorithmic gap indicator (Proposal 11). (6) RiskPremiumResidualMonitor — Q-world validation of ProxyDivergenceGuard (Proposal 12). (7) MarketSensitivityLibrary — standardized Greeks nomenclature with sensitivity-adjusted signals (Proposal 12). New modules: `sensitivity/`. 7 new unit tests. 7 new validation criteria.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| v5.2    | Added Proposal 13 & 14 (Quantitative Engine): Advanced statistical rigor (EVT, QR, D&I Reporting) integrated with 151 strategies (LegMatch, UniverseAggregator).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |

| v6      | Free data source migration: Polygon references replaced with Finnhub/Yahoo Finance. `PolygonWsClient` renamed to `EquityWsClient`. `PolygonTickCdmAdapter` renamed to `WsTickCdmAdapter`. Cross-validation references updated from OpenBB to FinanceToolkit/Yahoo Finance. `intraday_proxy.source` changed from `openbb_treasury_3m` to `fred_dgs3mo`. |

---

## Appendix: 01-computation-engine.md

> *Merged from `gaps/01-computation-engine.md` / `done/01-computation-engine.md` during plan_v6 consolidation.*
