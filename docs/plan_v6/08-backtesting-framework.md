...
- **Dynamic weight replay (Finding 6):** Replays periods where components had zero variance.
  ...
  in backtest output.
- **Historical data availability (v6):** Historical benchmark data (S&P 500 Shiller 1871-present, VIX 1990-present)
  now available via DataHub backfill (v6 Phase 7). Deep historical equity OHLCV (20+ years) available via
  Alpha Vantage (v6 Phase 6).
...
- Cross-validated via:
    ...
    - FinanceToolkit Sharpe ratio (analytics worker).
...
- MiFID II-style Stressed Market Condition (SMC) scenarios:
    - Message Spikes: 10x increase in tick volume from Finnhub.
...
- [ ] Cross-validated Sharpe agrees across TA-Lib, FinanceToolkit, and Yahoo Finance.
...
| v6      | `08-backtesting-framework.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Replaced OpenBB `obb.quantitative.sharpe_ratio` reference with FinanceToolkit Sharpe ratio (analytics worker). Replaced Polygon tick volume with Finnhub tick volume. Replaced "TA-Lib, FinanceToolkit, and OpenBB" with "TA-Lib, FinanceToolkit, and Yahoo Finance" in cross-validation. Added historical data availability notes: S&P 500 Shiller (1871-present) and VIX (1990-present) via DataHub backfill; deep historical equity OHLCV (20+ years) via Alpha Vantage. No structural changes to backtesting components. |
