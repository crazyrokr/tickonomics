# Project Plan: Funding Market Monitor (High-Performance Edition) — v7
...
This version adds three project-level concerns on top of v6:

**1. Landing Page** (`tickonomics.io`)

A marketing landing page serving as the public entry point — separate from the analytics
dashboard. Includes hero section, live demo embed, recent signals showcase, and SEO.

**2. GitHub Actions CI/CD Pipeline**

Full build, test, and deployment pipeline for all four deployable artifacts: landing page,
analytics dashboard, backend, and analytics worker. Covers PR checks, merge-to-main
staging deploys, manual promotion to production, and demo environment maintenance.

**3. Demo / Verification Phase with Virtual Portfolio**

A dedicated verification phase where the platform runs against **real market data** but
trades against a **virtual balance and portfolio**. Used to validate signal quality and
system behavior end-to-end before any real capital is at stake. Includes paper trading
engine, performance tracking, and signal quality reporting.

### v6 Change Summary (carried forward)
...
| **Frontend**           | Next.js 15, TailwindCSS, TypeScript             | Analytics dashboard (app.tickonomics.io) and landing page (tickonomics.io) |
...
### Phase 3: Landing Page (`tickonomics.io`)

- [ ] Implement marketing landing page (Next.js 15, separate app from analytics dashboard):
    - **Hero section:** tagline, one-sentence value proposition, "Start Demo" CTA button,
      animated ILI chart preview (static SVG or lightweight Lottie animation).
    - **Problem section:** three pain points — liquidity blind spots, lagging indicators,
      manual correlation analysis. Each with icon and one-paragraph explanation.
    - **How it works section:** three-step visual flow — "Ingest" (data sources icons:
      Fed Reserve, FRED, Polygon) → "Analyze" (ILI formula, correlation engine diagram)
      → "Act" (signal badges: ACTIONABLE, SPECULATIVE). Horizontal stepper with icons.
    - **Live demo section:** embedded read-only chart showing real ILI data from the
      staging environment (or cached snapshot if staging is down). Auto-refreshes daily.
    - **Signal showcase:** table of recent signals (symbol, direction, ILI percentile,
      timestamp, status badge) pulled from staging API. Read-only, no auth required.
    - **Virtual portfolio teaser:** live card showing demo portfolio performance — P&L,
      win rate, number of signals acted on — from the demo environment.
    - **Pricing/Access section:** "Open Source" badge, link to GitHub repo,
      self-hosted setup instructions, "Request Demo" contact form.
    - **Footer:** GitHub link, documentation link, disclaimer ("not financial advice").
- [ ] SEO and performance:
    - Server-side rendering with ISR (Incremental Static Regeneration), revalidate every 60s
      for live demo data.
    - OpenGraph meta tags, structured data (JSON-LD for SoftwareApplication).
    - Lighthouse target: Performance > 90, Accessibility > 95, SEO > 95.
    - Responsive design: mobile-first, works on all screen sizes.
- [ ] Analytics: page views, CTA clicks, demo signups tracked via Plausible or Umami
  (privacy-focused, no cookies, GDPR-friendly).
- [ ] Landing page deployed separately from dashboard — different subdomain or route
  (`tickonomics.io` for landing, `app.tickonomics.io` for dashboard).

### Phase 4: Frontend and Visualization (Analytics Dashboard)
...
### Phase 6: Demo / Verification with Virtual Portfolio

This phase validates the entire system end-to-end using real market data but simulated
trading. No real capital is at risk. The demo environment runs continuously and serves
as both a verification tool and a public showcase.

- [ ] Implement `VirtualPortfolio` — simulated portfolio with virtual balance:
    - Initial balance configurable (default $100,000).
    - Supports market orders (buy/sell) at the signal price with configured slippage.
    ...
    - Calculates portfolio-level metrics: total P&L, realized P&L, unrealized P&L,
...
- [ ] Implement `PaperTradingEngine`:
    ...
    - For each `ACTIONABLE` signal, executes a simulated trade via `VirtualPortfolio`.
    ...
    - Exit rules: hold until opposing signal, or configurable stop-loss / take-profit.
    - Marks trades as `PAPER` in the signal log to distinguish from live signals.
- [ ] Implement `SignalQualityReport` — daily automated report on signal performance:
    - Signals generated vs. signals acted upon (filter-through rate).
    - Hit rate: percentage of signals where price moved in the expected direction within
    ...
    - Stored in TimescaleDB, accessible via API for dashboard and landing page.
- [ ] Implement demo mode configuration:
  ```yaml
  monitor:
    demo:
      enabled: true
  ...
  ```
- [ ] Demo dashboard page (separate from main dashboard, accessible without auth):
    ...
    - "Live" indicator showing the system is processing real data.
...
- [ ] Verification criteria before graduating from demo:
    - Minimum 90 calendar days of continuous demo operation.
    - Minimum 50 paper trades executed.
    - Signal hit rate > 55% (statistically significant with p < 0.05).
    - No system-level bugs: zero missed signals, zero data gaps > stale threshold.
    - Portfolio Sharpe ratio > 0.5 over the demo period.
    - Max drawdown < 20% of virtual balance.
    - All these criteria configurable and tracked in TimescaleDB.
...
- [ ] Unit tests (given-when-then):
    - Given `ACTIONABLE` buy signal, When paper trading enabled, Then virtual position opened at signal price +
      slippage.
...
Full build, test, and deployment pipeline for all deployable artifacts.

#### Pipeline Structure

```
┌─────────────┐     ┌──────────────┐     ┌──────────────┐     ┌─────────────┐
│  On PR      │     │ On merge to  │     │  Manual      │     │  Scheduled  │
│  (any branch)│     │   main       │     │  promotion   │     │  (weekly)   │
└──────┬──────┘     └──────┬───────┘     └──────┬───────┘     └──────┬──────┘
       │                    │                     │                    │
       ▼                    ▼                     ▼                    ▼
  PR Checks           Build & Push          Staging → Prod       Demo Report
  ──────────          ─────────────         ──────────────       ────────────
  lint                Docker build          Smoke tests          Signal quality
  unit tests          Docker push           DB migration          report generation
  integration         Deploy staging        Deploy landing        Load test
  consistency         Deploy landing        Deploy dashboard      (Gatling)
  landing build       Deploy dashboard      Notify team
  landing lighthouse  Deploy analytics
                      Notify team
```
...
jobs:
  backend:
    ...
    steps:
      ...
      - Run: ./gradlew consistencyTest  # Cross-stack consistency
...
jobs:
  build-and-push:
    ...
    steps:
      - Checkout
      - Login to Container Registry (GitHub Container Registry or Docker Hub)
...
Each deployment (staging or prod) follows this checklist:
...
- [ ] Containerization:
    ...
    - Dockerfile for analytics dashboard (Node.js build to nginx static serving).
    - Dockerfile for analytics worker (Python + OpenBB SDK + FinanceToolkit).
    - Dockerfile for landing page (Node.js build to nginx static serving).
    - `docker-compose.yml` for local dev (backend + dashboard + landing + analytics worker +
      TimescaleDB + Chronicle Queue volume mount).
    - `docker-compose.openbb.yml` — reference compose for OpenBB sidecar (user-managed).
    - `docker-compose.demo.yml` — demo environment with paper trading engine,
      pre-seeded historical data, and public-facing endpoints.
- [ ] Environment strategy:
    - dev: local docker-compose + user-managed OpenBB sidecar, Polygon free tier.
    - demo: cloud-deployed, real API keys, virtual portfolio, public read-only access.
      Runs Phase 6 paper trading continuously. Landing page pulls live demo data from here.
    - staging: cloud-deployed, real API keys, full data sync. Used for pre-production
      validation before promoting to prod.
    - prod: HA TimescaleDB (managed or replicated), nginx reverse proxy, OpenBB sidecar.
      Only deployed after demo verification criteria are met (Phase 6).
- [ ] CI/CD: See Phase 7 for full pipeline specification.
...
| ADR-020 | Landing page as separate Next.js application from analytics dashboard           | Proposed |
| ADR-021 | Demo/verification phase with virtual portfolio before live deployment           | Proposed |
| ADR-022 | GitHub Actions CI/CD with four deployable artifacts                             | Proposed |
