...
jobs:
  chaos-test:
    ...
    steps:
      ...
      - name: Scenario 4 - Polygon WS Disconnect
        run: |
          # Disconnect Polygon WebSocket for 6 minutes (> 5 min fallback threshold)
          # Verify: REST fallback activates, no tick data gaps
          chaos inject network-partition --target polygon-ws --duration 6m
...
| Polygon WS disconnect    | 6 min outage          | REST fallback activates after 5 min, `POLYGON_WS_FALLBACK` logged                        |
...
- [ ] Chaos scenario 4 (Polygon WS disconnect) verifies REST fallback activates after 5 min.
