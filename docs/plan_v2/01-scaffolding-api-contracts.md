...
├── ingestion/             # FRED/NY Fed direct clients, Polygon WebSocket client, OpenBB client
...
| v3      | Added "CDM Adapter Layer" (Proposal #5) as new deliverable section. Added `CdmAdapter<T>` interface, per-source adapters, CDM output types. Updated `FederationDataClient` to return source-specific raw types. Updated `ingestion/` in project structure to include CDM adapters.                                                                    |