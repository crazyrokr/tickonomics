"""Autoencoder anomaly model (torch import deferred to this private module).

Kept separate from ``anomaly_service`` so importing the public service does not load torch: torch loads
only when training/detection is actually invoked. See ADR-036 D1.
"""
...
def detect(
    ...
    try:
        ...
        state_dict = {
            key: torch.tensor(value, dtype=torch.float32) for key, value in parsed.items()
        }
    except Exception:
        return {"error": "Invalid model state"}
