...
public class IngestionDlqRepository {
  ...
  public void save(IngestionDlqEntry entry) {
    jdbc.update(
        "INSERT INTO ingestion_dlq (source, payload, error_message, idempotency_key) VALUES (:source, "
            + ":payload::jsonb, :errorMessage, :idempotencyKey)",
        toParams(entry));
  }

  public List<IngestionDlqEntry> findUnreplayed(int limit) {
    return jdbc.query(
        "SELECT created_at, source, payload, error_message, idempotency_key FROM ingestion_dlq WHERE NOT replayed "
        ...
        (rs, rowNum) -> new IngestionDlqEntry(
            ...
            rs.getObject("idempotency_key") != null ? java.util.UUID.fromString(rs
