...
public class CorrelationOutputRepository {
    ...
    public void save(CorrelationOutput output) {
        jdbc.update(
                "INSERT INTO correlation_outputs (time, symbol, metric, correlation, p_value, sample_size, lag_order, direction) " +
                        "VALUES (:time, :symbol, :metric, :correlation, :pValue, :sampleSize, :lagOrder, :direction)",
                toParams(output));
    }

    public List<CorrelationOutput> findBySymbolAndMetricAndTimeBetween(String symbol, String metric, Instant from, Instant to) {
        return jdbc.query(
                "SELECT time, symbol, metric, correlation, p_value, sample_size, lag_order, direction " +
                        "FROM correlation_outputs WHERE symbol = :symbol AND metric = :metric AND time BETWEEN :from AND :to ORDER BY time",
