...
public class ZscoreSeriesRepository {
    ...
    public void save(ZscoreSeries entry) {
        jdbc.update(
                "INSERT INTO zscore_series (time, component, raw_value, z_score, lookback_days) VALUES (:time, :component, :rawValue, :zScore, :lookbackDays)",
                toParams(entry));
    }
    ...
    public void saveAll(List<ZscoreSeries> entries) {
        jdbc.batchUpdate(
                "INSERT INTO zscore_series (time, component, raw_value, z_score, lookback_days) VALUES (:time, :component, :rawValue, :zScore, :lookbackDays)",
                SqlParameterSourceUtils.createBatch(entries.stream().map(this::toParams).toArray(MapSqlParameterSource[]::new)));
    }

    public List<ZscoreSeries> findByComponentAndTimeBetween(String component, Instant from, Instant to) {
        return jdbc.query(
                "SELECT time, component, raw_value, z_score, lookback_days FROM zscore_series WHERE component = :component AND time BETWEEN :from AND :to ORDER BY time",
