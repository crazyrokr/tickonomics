...
public class RateSnapshotRepository {
    ...
    public void saveAll(List<RateSnapshot> snapshots) {
        jdbc.batchUpdate(
                "INSERT INTO rate_snapshots (time, rate_type, value, source) VALUES (:time, :rateType, :value, :source)",
                SqlParameterSourceUtils.createBatch(snapshots.stream().map(this::toParams).toArray(MapSqlParameterSource[]::new)));
    }

    public List<RateSnapshot> findByRateTypeAndTimeBetween(String rateType, Instant from, Instant to) {
        return jdbc.query(
                "SELECT time, rate_type, value, source FROM rate_snapshots WHERE rate_type = :rateType AND time BETWEEN :from AND :to ORDER BY time",
    ...
    public List<RateSnapshot> findLatestByRateType(String rateType, int limit) {
        return jdbc.query(
                "SELECT time, rate_type, value, source FROM rate_snapshots WHERE rate_type = :rateType ORDER BY time DESC LIMIT :limit",
