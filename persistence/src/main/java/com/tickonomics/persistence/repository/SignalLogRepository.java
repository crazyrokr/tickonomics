...
public class SignalLogRepository {
    ...
    public long save(SignalLog signal) {
        ...
        jdbc.update(
                "INSERT INTO signal_log (created_at, symbol, direction, status, ili_percentile, ili_value, expected_move, estimated_cost, signal_metadata) " +
                        "VALUES (:createdAt, :symbol, :direction, :status, :iliPercentile, :iliValue, :expectedMove, :estimatedCost, :signalMetadata::jsonb)",
                toParams(signal), keyHolder, new String[]{"id"});
        return keyHolder.getKey().longValue();
    }

    public List<SignalLog> findBySymbolAndTimeBetween(String symbol, Instant from, Instant to) {
        return jdbc.query(
                "SELECT created_at, symbol, direction, status, ili_percentile, ili_value, expected_move, estimated_cost, signal_metadata " +
