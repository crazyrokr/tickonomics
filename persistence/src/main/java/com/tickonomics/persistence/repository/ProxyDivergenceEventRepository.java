...
public class ProxyDivergenceEventRepository {
    ...
    public void save(ProxyDivergenceEvent event) {
        jdbc.update(
                "INSERT INTO proxy_divergence_events (detected_at, sofr_value, tbill_proxy_value, correlation_5d, divergence_score, resolution, resolved_at) " +
                        "VALUES (:detectedAt, :sofrValue, :tbillProxyValue, :correlation5d, :divergenceScore, :resolution, :resolvedAt)",
                toParams(event));
    }
